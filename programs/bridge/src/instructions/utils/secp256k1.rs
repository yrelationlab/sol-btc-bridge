...
use solana_program::{instruction::Instruction, sysvar::instructions};
...
pub fn resolve_secp256k1_with_index<'a>(
    ...
    let eth_address_bytes: Vec<u8> = vec![/* your Ethereum address bytes */];

    let msg: Vec<u8> = vec![/* your message bytes */];
    let sig: Vec<u8> = vec![/* your signature bytes */];
    let recovery_id: u8 = 27; // Example recovery ID, typically 0 or 1

     // Check that ix is what we expect to have been sent
    verify_secp256k1_ix(&ix, &eth_address_bytes, &msg, &sig, recovery_id)?;

    let eth_address_array: [u8; 32] = eth_address_bytes.try_into().map_err(|_| ErrorCode::SigVerificationFailed)?;
    let eth_address = Pubkey::from(eth_address_array);
    return Ok((eth_address, msg));
}
